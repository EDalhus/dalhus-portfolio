/* ==========================================================================
   ...
   createGallery() er en factory slik at samme logikk kan drive flere
   uavhengige galleri-vinduer (siste bilder, et bestemt album, …) — hver
   med sin egen data, forhåndsvisning og scroll-tilstand.
   ========================================================================== */
 ...
 * @param {string} options.bodyId       — id på elementet innholdet rendres inn i
 * @param {string} options.statusId     — id på statuslinje-feltet
 * @param {object} [options.extraParams] — ekstra spørreparametre til /api/smugmug (f.eks. { album: "/FA/KANDU/TG26H" })
 */
export function createGallery({ bodyId, statusId, extraParams = {} }) {
  let data = null;
  ...
  async function fetchPage(count, offset) {
    const params = new URLSearchParams({ images: String(count), offset: String(offset), ...extraParams });
    const res = await fetch(`${GALLERY.endpoint}?${params.toString()}`, {
  ...
  function photoTile(image, index) {
    ...
    const meta = formatDate(image.date, lang);
  ...
  function applyPreviewChrome() {
    ...
    previewBackdrop
      ...
      ?.setAttribute("aria-label", dict["gallery.prev"]);
    previewBackdrop
      ?.querySelector(".preview-next")
      ?.setAttribute("aria-label", dict["gallery.next"]);
  ...
  function renderPreview() {
    ...
    previewMetaEl.textContent = formatDate(image.date, lang);
  ...
  function buildScaffold() {
    const root = document.querySelector(`#${bodyId}`);
    if (!root) return;
